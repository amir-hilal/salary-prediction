...
from src.models.train import load_pipeline, save_pipeline, train
...
def trained() -> tuple[Pipeline, dict, pd.DataFrame]:
    df = _make_synthetic_df()
    pipeline, best_params = train(df[FEATURE_COLUMNS], df[TARGET_COLUMN])
    return pipeline, best_params, df
...
def test_save_pipeline_creates_file(trained: tuple, tmp_path: pytest.TempPathFactory) -> None:
    pipeline, _, _ = trained
    artifact_path = save_pipeline(pipeline, tmp_path)
...
def test_load_pipeline_roundtrip_produces_same_predictions(
    ...
    pipeline, _, df = trained
    artifact_path = save_pipeline(pipeline, tmp_path)
    loaded = load_pipeline(artifact_path)

    original_pred = pipeline.predict(df[FEATURE_COLUMNS][:5])
    loaded_pred = loaded.predict(df[FEATURE_COLUMNS][:5])
